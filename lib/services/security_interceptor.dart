...
class SecurityInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    if (options.uri.scheme != 'https' && SecurityConfig.isProduction) {
      AppLogger.warning(
          'SECURITY ALERT: Blocked insecure HTTP request to ${options.uri}');
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) {
    ...
    if (response.statusCode != 200 && response.statusCode != 201) {
      AppLogger.warning(
          'Suspicious Response: ${response.statusCode} from ${response.requestOptions.path}');
    }
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    ...
    final sanitizedMessage = err.message
        ?.replaceAll(RegExp(r'AIza[0-9A-Za-z-_]{35}'), '[REDACTED_KEY]');
